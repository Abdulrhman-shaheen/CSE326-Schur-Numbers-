...
__global__ void processLevel(State* current, int count, State* next, 
    ...
    if (idx >= count || *next_count >= max_states) return;
    ...
    for (int c = 0; c < 5; ++c) {
        ...
        if (valid) {
            ...
            if (pos < max_states) next[pos] = new_state;
        }
    }
}
...
int main() {
    ...
    set_bit(initial.colors[2], 3);  // Green: 3
    set_bit(initial.colors[3], 4);  // Cyan: 4
    set_bit(initial.colors[4], 5);  // Magenta: 5
